...
function objgizmo(handle, boneid, dict, anim)
    spawnedProp = handle
    pedBoneId = boneid
    ...
    local rotationString = string.format("%.2f, %.2f, %.2f", rotation.x, rotation.y, rotation.z)
    print("Rotations: " .. rotation.x, rotation.y, rotation.z)


    TriggerServerEvent("webhook", rotationString)
...
--Example: /prop prop_cs_burger_01 18905 mp_player_inteat@burger mp_player_int_eat_burger
RegisterCommand('prop',function(source, args, rawCommand)
    ...
    local objectPositionData = objgizmo(object, bone, args[3], args[4])
end)
